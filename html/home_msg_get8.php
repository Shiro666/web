...
//home_msg_get?.php的性质是一样的，这一个是wechat诈骗
...
include("connectDB.php");
    ...
    $i=0;
	$sql="SELECT case_id,title,content FROM wechat order by date desc limit 1";
	$result=$db->query($sql);	
	if($result->num_rows>0){
    	while($row=$result->fetch_assoc()){    		
			...
			$output[$i]['pic']="images/type8pic.webp";			
			$i++;   		
    	}
    }
	
	$sql="SELECT case_id,title,date FROM wechat order by date desc limit 1,6";
	$result=$db->query($sql);
	if($result->num_rows>0){
    	while($row=$result->fetch_assoc()){
    		$output[$i]["case_id"]=$row["case_id"];
